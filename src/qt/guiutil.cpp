...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("preppercoin"))
        return false;
...
{
    // Convert preppercoin:// to preppercoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("preppercoin://"))
    {
        uri.replace(0, 12, "preppercoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PrepperCoin.lnk";
}
...
{
    return GetAutostartDir() / "preppercoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PrepperCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  preppercoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PrepperCoin-Qt"));
